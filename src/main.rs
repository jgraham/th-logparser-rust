...
#[derive(Debug, RustcEncodable, Clone, Copy)]
struct ErrorLine<'a> {
    linenumber: u32,
    line: &'a str
}
...
impl<'a> ErrorLine<'a> {
    fn new(line_number: u32, line: &'a str) -> ErrorLine<'a> {
        ErrorLine {
...
trait LogParser<'a> {
    fn parse_line(&mut self, line: &'a str, line_number: u32);
...
struct StepData<'a> {
    steps: Vec<Step<'a>>,
    all_errors:Vec<ErrorLine<'a>>, //TODO: Try making this a reference to avoid a copy
...
enum StepState<'a> {
    AwaitingFirstStep,
    StepInProgress(Step<'a>),
    StepFinished
}

impl <'a> StepState<'a> {
    fn unwrap(self) -> Step<'a> {
        match self {
...
impl <'a> StepParser<'a> {
    ...
    fn start_step(&mut self, line_number: u32,
                  name: Option<&'a str>,
    ...
    fn end_step(&mut self,
        ...
        self.artifact.all_errors.extend(step.errors.iter());
        self.artifact.steps.push(step)
    }

    fn parse_error(&mut self, line: &'a str, line_number: u32, trimmed: &'a str) {
        match self.state {
...
impl<'a> LogParser<'a> for StepParser<'a> {
    fn parse_line(&mut self, line: &'a str, line_number: u32) {
...
#[derive(RustcEncodable)]
struct StepDataArtifact<'a> {
    step_data: StepData<'a>
}

impl<'a> StepDataArtifact<'a> {
...
fn main() {
    ...
    let artifact = StepDataArtifact::new(parser.artifact);
    println!("{}", json::encode(&artifact).unwrap());
